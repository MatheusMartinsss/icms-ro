...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (!cte.chave) return NextResponse.json({ error: 'CT-e sem chave de acesso' }, { status: 422 })
    ...
    let res: Response
    try {
        res = await fetch(`${MICROSERVICE_URL}/api/v1/cte/${cte.chave}/cancel`, {
            method: 'POST',
            headers: {
    ...
    return NextResponse.json({ cstat: data.cstat, xmotivo: data.xmotivo })
}
